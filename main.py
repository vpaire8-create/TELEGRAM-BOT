                ...
                try:
                    ...
                    if is_editable:
                        ...
                        if any(keyword in element_text for keyword in keywords):
                            ...
                            log_message(f'{process_id}: Using primary selector editable element', automation_state)
                            return element
...
async def send_messages_from_file(token, tid, hater_name, speed, file_content, chat_id, context, user_id, automation_type='group'):
    ...
    try:
        if automation_type == 'inbox':
            # Use Selenium for inbox/group messages
            config = {
                'cookies': '',  # You can add cookie support if needed
                'tid': tid,
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.user_data.get('automation_type') == 'inbox':
        if context.user_data['step'] == 'waiting_for_token':
            ...
            # Split if too long
            if len(group_list) > 4096:
                group_list = group_list[:4000] + "\n... (truncated)"
